...
Definition derives (Gamma : theory) (J : judgment) : Prop :=
  exists Xi : closure_certificate Gamma J, True.
...
Theorem derives_monotone :
  forall Gamma Delta J,
    ...
    derives Delta J.
Proof.
  intros Gamma Delta J Hsub [Xi _].
  exists {|
    closure_premises := closure_premises Gamma J Xi;
    closure_premises_ok :=
      fun G HG =>
...
Proof.
  ...
  - destruct (Hder K (or_introl eq_refl)) as [XiK _].
    destruct
...
Proof.
  intros phi [[Xi_true _] [Xi_false _]].
  assert
    (Htrue_assumptions :
      forall G,
