...
Definition separating_type : ty := Base (TermSort 0).

Definition separating_protocol : protocol separating_type.
Proof.
  refine {|
    raw_obs := fun r => r = RecRed [];
    proto_equiv := fun r s => r = s;
    allowed_ctx := fun _ => False;
    allowed_comp := fun _ _ => False;
    allowed_iter := fun _ _ => False;
    allowed_red := fun _ => False;
    allowed_int := fun _ => False
  |}.
  - intros r Hr. subst r. constructor. constructor.
  - constructor.
    + intro r. reflexivity.
    + intros r s H. symmetry. exact H.
    + intros r s t Hrs Hst. now transitivity s.
  - intros C H. contradiction.
  - intros r s H. contradiction.
  - intros n r H. contradiction.
  - intros rs H. contradiction.
  - intros rs H. contradiction.
Defined.

Lemma separating_closure_exact :
  forall r, obs_closure separating_protocol r -> r = RecRed [].
Proof.
  fix IH 2.
  intros r Hobs.
  destruct Hobs as
      ...
      | rs Hallow Hrs].
  - exact Hraw.
  - simpl in Heq. subst s. exact (IH r Hr).
  - simpl in Hallow. contradiction.
  - simpl in Hallow. contradiction.
  - simpl in Hallow. contradiction.
  - simpl in Hallow. contradiction.
  - simpl in Hallow. contradiction.
...
Theorem separating_protocol_nontrivial :
  obs_closure separating_protocol (RecRed []) /\
  ~ obs_closure separating_protocol (RecInt []).
Proof.
  split.
  - apply ObsRaw. reflexivity.
  - intro Hbad.
    apply separating_closure_exact in Hbad.
    discriminate Hbad.
Qed.
  ...
  fis_free_core :
    forall tau f A HA HC,
      ...
      (forall theta r,
          ...
          obs_closure (free_protocol tau A HA HC) (subst_record theta r)) ->
      legal_interpretation;
  fis_model_protocol : protocol separating_type;
  fis_model_nontrivial :
    obs_closure fis_model_protocol (RecRed []) /\
    ~ obs_closure fis_model_protocol (RecInt []);
  fis_enum_sound :
    forall D e j,
      enum_result D e j ->
      exists tau f A E,
...
Definition FIS_sharp : FIS_system := {|
  ...
  fis_model_protocol := separating_protocol;
  fis_model_nontrivial := separating_protocol_nontrivial;
