...
Definition id_occurs (n : nat) (ids : list nat) : bool :=
  existsb (fun m => Nat.eqb n m) ids.

Record MatchCert (d : Diagram) (r : RewriteRule) : Type := {
  match_pairs : list (nat * nat);
  match_left_ids_unique : NoDup (map fst match_pairs);
  match_target_ids_unique : NoDup (map snd match_pairs);
  match_left_total :
    forall l,
      In l (left_side r) ->
      exists c,
        In c (dcomps d) /\
        In (cid l, cid c) match_pairs /\
        SameComponentShape l c;
  match_pairs_sound :
    forall lid did,
      In (lid, did) match_pairs ->
      exists l c,
        In l (left_side r) /\ cid l = lid /\
        In c (dcomps d) /\ cid c = did /\
        SameComponentShape l c
}.

Definition matched_target_ids {d r} (m : MatchCert d r) : list nat :=
  map snd (match_pairs d r m).

Definition unmatched_components {d r} (m : MatchCert d r) : list Component :=
  filter
    (fun c => negb (id_occurs (cid c) (matched_target_ids m)))
    (dcomps d).
...
Record DangCert (d : Diagram) (r : RewriteRule) : Type := {
  ...
  dangling_closed :
    forall c l did,
      In c (dcomps d) ->
      ~ In (cid c) (matched_target_ids dang_match) ->
      In l (left_side r) ->
      In (cid l, did) (match_pairs d r dang_match) ->
      ~ In did (cdeps c)
}.
...
Record InstCert (d : Diagram) (r : RewriteRule) : Type := {
  ...
  inst_side_condition : side_condition r d;
  inst_added_exact : inst_added = right_side r;
  inst_result_exact :
    inst_result = unmatched_components (dang_match d r inst_dang) ++ inst_added;
...
Fixpoint max_depth (xs : list Component) : nat :=
  match xs with
  | [] => 0
  | c :: rest => Nat.max (crd c) (max_depth rest)
  end.

Definition diagram_height (d : Diagram) : nat := max_depth (dcomps d).

Record TSLModel : Type := {
  observation_carrier : Type;
  observe_diagram : Diagram -> observation_carrier
}.

Definition depth_observation_model : TSLModel := {|
  observation_carrier := nat;
  observe_diagram := diagram_height
|}.

Definition SemanticallyDistinct (d e : Diagram) : Prop :=
  exists M : TSLModel,
    observe_diagram M d <> observe_diagram M e.

Lemma singleton_diagram_height :
  forall k, diagram_height (singleton_diagram_at_depth k) = k.
Proof.
  intro k.
  unfold diagram_height, singleton_diagram_at_depth.
  simpl.
  apply Nat.max_0_r.
Qed.

Theorem depth_model_is_nontrivial :
  SemanticallyDistinct
    (singleton_diagram_at_depth 0)
    (singleton_diagram_at_depth 1).
Proof.
  exists depth_observation_model.
  simpl.
  rewrite !singleton_diagram_height.
  discriminate.
Qed.
Record TopologicalSchematicLogicSystem : Type := {
  ...
  tsl_models : Type;
  tsl_depth_model : tsl_models;
  tsl_semantic_nontriviality :
    SemanticallyDistinct
      (singleton_diagram_at_depth 0)
      (singleton_diagram_at_depth 1)
}.
...
Definition TSL : TopologicalSchematicLogicSystem := {|
  ...
  tsl_models := TSLModel;
  tsl_depth_model := depth_observation_model;
  tsl_semantic_nontriviality := depth_model_is_nontrivial
|}.
