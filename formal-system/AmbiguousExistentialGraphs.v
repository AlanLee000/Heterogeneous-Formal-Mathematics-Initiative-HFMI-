...
Definition empty_relation_structure : structure := {|
  rel_denote := fun _ _ => False
|}.

Lemma empty_relation_structure_models_top :
  models_sentence empty_relation_structure FTop.
Proof.
  split.
  - split; reflexivity.
  - intro g. exact I.
...
Lemma empty_relation_structure_refutes_bottom :
  inhabited Dom ->
  ~ models_sentence empty_relation_structure FBot.
Proof.
  intros [d] [_ Hall].
  exact (Hall (fun _ => d)).
Qed.
...
Record formal_system (tau : formula -> ceg) : Type := {
  ...
  fs_counter : forall A : aeg_framework, af_strategy A -> structure -> Prop :=
    strategy_counter;
  fs_semantic_model : structure;
  fs_semantic_top : models_sentence fs_semantic_model FTop;
  fs_semantic_bottom_refuted : ~ models_sentence fs_semantic_model FBot;
  fs_beta_pattern : Type;
  fs_beta_pattern_diagram : fs_beta_pattern -> aeg
}.

Definition build_formal_system
    (tau : formula -> ceg) (Htau : rep_tau tau)
    (HDom : inhabited Dom) : formal_system tau := {|
  fs_rep_tau := Htau;
  ...
  fs_semantic_model := empty_relation_structure;
  fs_semantic_top := empty_relation_structure_models_top;
  fs_semantic_bottom_refuted :=
    empty_relation_structure_refutes_bottom HDom;
