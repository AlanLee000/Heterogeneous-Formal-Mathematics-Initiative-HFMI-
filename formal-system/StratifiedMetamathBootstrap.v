...
Lemma line_ok_is_omega :
  forall gamma seen a,
    LineOK gamma seen a -> FrmOmega a.
Proof.
  intros gamma seen a Hline.
  inversion Hline; assumption.
Qed.

Lemma lines_ok_all_omega :
  forall gamma seen pi,
    LinesOK gamma seen pi -> Forall FrmOmega pi.
Proof.
  intros gamma seen pi. revert seen.
  induction pi as [|a rest IH]; intros seen Hlines.
  - constructor.
  - cbn in Hlines. destruct Hlines as [Hline Hrest].
    constructor.
    + exact (line_ok_is_omega gamma seen a Hline).
    + exact (IH (seen ++ [a]) Hrest).
Qed.

Lemma last_formula_some_in :
  forall pi a,
    last_formula pi = Some a -> In a pi.
Proof.
  induction pi as [|b rest IH]; intros a Hlast.
  - discriminate.
  - destruct rest as [|c tail].
    + cbn in Hlast. inversion Hlast. subst. cbn. auto.
    + cbn in Hlast. right. exact (IH a Hlast).
Qed.

Theorem derivable_is_omega :
  forall gamma a,
    Derivable gamma a -> FrmOmega a.
Proof.
  intros gamma a [pi [[Hnonempty Hlines] Hlast]].
  pose proof (lines_ok_all_omega gamma [] pi Hlines) as Hall.
  rewrite Forall_forall in Hall.
  exact (Hall a (last_formula_some_in pi a Hlast)).
Qed.

Definition layer_mismatch_body : Frm := FDom 1 (TVar 0).
Definition layer_mismatch_term : Term :=
  TQuote 0 0 layer_mismatch_body.
Definition layer_mismatch_formula : Frm :=
  FDom 0 layer_mismatch_term.

Lemma layer_mismatch_body_not_at_zero :
  ~ FrmAt 0 layer_mismatch_body.
Proof.
  unfold layer_mismatch_body.
  intro H. inversion H; subst; lia.
Qed.

Lemma term_at_quote_requires_body_level :
  forall m n x a,
    TermAt m (TQuote n x a) -> FrmAt n a.
Proof.
  intros m n x a Hterm.
  remember (TQuote n x a) as q eqn:Heq.
  induction Hterm.
  - discriminate.
  - apply IHHterm. exact Heq.
  - inversion Heq. subst. assumption.
Qed.

Lemma layer_mismatch_term_not_omega :
  ~ TermOmega layer_mismatch_term.
Proof.
  intros [m Hterm].
  apply layer_mismatch_body_not_at_zero.
  exact (term_at_quote_requires_body_level m 0 0
    layer_mismatch_body Hterm).
Qed.

Lemma frm_at_dom_requires_term_omega :
  forall n k t,
    FrmAt n (FDom k t) -> TermOmega t.
Proof.
  intros n k t Hfrm.
  remember (FDom k t) as a eqn:Heq.
  induction Hfrm; try discriminate.
  - inversion Heq. subst.
    exists n. assumption.
  - apply IHHfrm. exact Heq.
Qed.

Lemma layer_mismatch_formula_not_omega :
  ~ FrmOmega layer_mismatch_formula.
Proof.
  intros [n Hfrm].
  apply layer_mismatch_term_not_omega.
  exact (frm_at_dom_requires_term_omega n 0
    layer_mismatch_term Hfrm).
Qed.

Theorem stratified_bootstrap_syntactically_nontrivial :
  ~ TheoremF layer_mismatch_formula.
Proof.
  intro Hthm.
  apply layer_mismatch_formula_not_omega.
  exact (derivable_is_omega (fun _ => False)
    layer_mismatch_formula Hthm).
Qed.
...
Record StratifiedMetamathSystem : Type := {
  ...
  sys_derivable_is_omega :
    forall gamma a, Derivable gamma a -> FrmOmega a;
  sys_syntactic_nontrivial :
    ~ TheoremF layer_mismatch_formula
}.
...
Definition T_omega_star : StratifiedMetamathSystem := {|
  ...
  sys_derivable_is_omega := derivable_is_omega;
  sys_syntactic_nontrivial :=
    stratified_bootstrap_syntactically_nontrivial
|}.
