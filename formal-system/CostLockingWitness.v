...
Record ledger_model : Type := {
  interpret_ledger : ledger -> cost_carrier K
}.

Definition canonical_ledger_model : ledger_model := {|
  interpret_ledger := ledger_cost
|}.

Theorem ledger_model_nontrivial :
  forall rho,
    interpret_ledger canonical_ledger_model ledger_empty <>
    interpret_ledger canonical_ledger_model (ledger_single rho).
Proof.
  intros rho Heq.
  simpl in Heq.
  rewrite cost_plus_zero_r in Heq.
  exact (proj2_sig (resp_charge rho) (eq_sym Heq)).
Qed.
Record formal_system_tuple : Type := {
  ...
  fs_model : ledger_model;
  fs_model_nontrivial :
    forall rho,
      interpret_ledger fs_model ledger_empty <>
      interpret_ledger fs_model (ledger_single rho);
...
Definition canonical_formal_system_tuple : formal_system_tuple := {|
  ...
  fs_model := canonical_ledger_model;
  fs_model_nontrivial := ledger_model_nontrivial;
  fs_deriv_v := deriv_v;
  fs_deriv_c := deriv_c;
