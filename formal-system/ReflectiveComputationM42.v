...
Record transition_model : Type := {
  transition_state : Type;
  transition_relation : transition_state -> transition_state -> Prop
}.

Definition operational_transition_model : transition_model := {|
  transition_state := Term;
  transition_relation := StepM
|}.

Lemma comm_T0_T1_distinct : comm_T0 <> comm_T1.
Proof.
  unfold comm_T0, comm_T1, comm_data0, comm_data1.
  discriminate.
Qed.

Theorem operational_model_nontrivial :
  exists T T',
    transition_relation operational_transition_model T T' /\ T <> T'.
Proof.
  exists comm_T0, comm_T1.
  split.
  - exact comm_first_stepM.
  - exact comm_T0_T1_distinct.
Qed.
Definition CounterMachineOneStepEmbedding : Prop :=
  forall M c c',
    CM_step M c c' ->
    Step (machine_step_term c c') (machine_step_term_next c c').

Theorem counter_machine_one_step_simulation : CounterMachineOneStepEmbedding.
Proof.
  intros M c c' H.
...
Definition CounterMachineOneStepEmbeddingM : Prop :=
  forall M c c',
    CM_step M c c' ->
    StepM (machine_step_term c c') (machine_step_term_next c c').

Theorem counter_machine_one_step_simulation_source : CounterMachineOneStepEmbeddingM.
Proof.
  intros M c c' Hstep.
  exact (certified_step_is_stepM
    (@counter_machine_one_step_simulation M c c' Hstep)).
Qed.
...
Record MSystem : Type := {
  ...
  ms_model : transition_model := operational_transition_model;
  ms_model_nontrivial :
    exists T T', transition_relation ms_model T T' /\ T <> T'
    := operational_model_nontrivial;
  ms_counter_machine_one_step_embedding : CounterMachineOneStepEmbeddingM
