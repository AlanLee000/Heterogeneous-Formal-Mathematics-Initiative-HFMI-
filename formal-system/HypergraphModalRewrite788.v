...
Lemma primitive_rewrite_is_semantic :
  forall v t s, rho v t s -> semantic_rw v t s.
Proof.
  intros v t s Hstep.
  apply syntactic_soundness.
  unfold syntactic_node_rw, syntactic_derives.
  apply derives_to_sequence.
  eapply derives_step.
  - apply Rule_Base. exact Hstep.
  - constructor.
Qed.

Theorem semantic_nontrivial_if_primitive :
  primitive_rewrite_nontrivial ->
  exists v t s, semantic_rw v t s /\ t <> s.
Proof.
...
Record HMRSystem : Type := {
  ...
  hmr_soundness :
    ...
    := syntactic_soundness;
  hmr_primitive_nontrivial : Prop := primitive_rewrite_nontrivial;
  hmr_semantic_nontrivial :
    primitive_rewrite_nontrivial ->
    exists v t s, semantic_rw v t s /\ t <> s
