...
Inductive finite_model_rule : Type :=
| NoConcrete
| NoReflect
| NoElim.

Record finite_countermodel : Type := {
  carrier_size : nat;
  missing_rule : finite_model_rule;
  unit_value : nat;
  equiv_value : nat -> nat -> nat;
  failed_instance : term * term;
  retained_rules_valid : Prop;
  target_rule_fails : Prop
}.

Definition rule_independence_statement : Prop :=
  exists M1 M2 M3,
    missing_rule M1 = NoConcrete /\
    missing_rule M2 = NoReflect /\
    missing_rule M3 = NoElim /\
    target_rule_fails M1 /\
    target_rule_fails M2 /\
    target_rule_fails M3.
